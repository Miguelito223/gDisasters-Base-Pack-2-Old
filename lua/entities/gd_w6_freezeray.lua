...
ENT.PrintName		                 =  "Like Really Cold"
ENT.Author			                 =  "Hmm"
ENT.Contact		                     =  "Hmm"
ENT.Category                         =  "Hmm"
...
function ENT:Initialize()		
	...
	if (SERVER) then
		...
		GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(35,38),["Direction"]=Vector(0,0,0)}, ["Pressure"]    = 10000, ["Temperature"] = math.random(-300,-355), ["Humidity"]    = math.random(0,0), ["BRadiation"]  = 0.1}}
	...
	for k, v in pairs(player.GetAll()) do
		...
		if v.gDisasters.Area.IsOutdoor then
			...
			if math.random(1,50)==50 then
				...
				net.WriteString("localized_blizzard_effect", Angle(0,math.random(1,40),0))
				net.Send(v)			
			end
		end
	end
